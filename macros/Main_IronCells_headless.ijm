...
objectCsvNamed = outputDir + "/cell_features_" + originalFileName + ".csv";
...
File.saveString(cellFeatureHeader, objectCsvNamed);
...
for (i = 0; i < nObjects; i++) {
    ...
    if (accepted == 1) {
        ...
        File.append(objectLine, objectCsvNamed);
...
if (saveOverlays == 1) {
    ...
    saveAs("Tiff", outputDir + "/cellmask_" + originalFileName + ".tif");
    selectWindow("Original_RGB");
    ...
    saveAs("Png", outputDir + "/vis_cellpixels_" + originalFileName + ".png");
    selectWindow("Blue_Pixels_Mask");
    saveAs("Tiff", outputDir + "/blue_inside_cells.tif");
    saveAs("Tiff", outputDir + "/blue_inside_cells_" + originalFileName + ".tif");
...
if (saveOverlays == 1) {
    ...
    saveAs("Jpeg", outputDir + "/roi_overlay_" + originalFileName + ".jpg");
...
summaryCsv = outputDir + "/frame_features.csv";
summaryCsvNamed = outputDir + "/frame_features_" + originalFileName + ".csv";
...
File.saveString(summaryHeader, summaryCsvNamed);
...
File.append(summaryLine, summaryCsvNamed);
